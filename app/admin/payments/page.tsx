...
export default function AdminPaymentsPage() {
  ...
  async function save() {
    ...
    setSaving(false)
    setMsg(res.ok ? 'Payment settings saved.' : 'Failed to save.')
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <p className="text-sm text-[#6b5555] mb-6">
        Enable gateways and wallets for user deposits. Credentials are stored in site settings.
      </p>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 max-w-xl bg-white rounded-2xl border border-[#f0e0e0] p-5 sm:p-6 space-y-4">
          ...
          <div className="border-t border-[#f0e0e0] pt-4 space-y-3">
            ...
            <div>
              <label className="text-xs text-[#6b5555]">Fee % (optional)</label>
              <input
          ...
          {msg && <p className="text-sm text-[#5c4040]">{msg}</p>}
          <button
            type="button"
            onClick={save}
            disabled={saving}
