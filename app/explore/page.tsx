...
export default async function ExplorePage({
  ...
  return (
    <main className="min-h-screen bg-[#f4f5f7] overflow-x-hidden">
      <div className="container mx-auto px-3 sm:px-4 py-8 max-w-6xl">
        ...
        <form className="bg-white rounded-2xl border border-slate-200 p-3 sm:p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 min-w-0">
          <input
            name="q"
            defaultValue={q}
            placeholder="Search title, @username, tags…"
            className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm sm:col-span-2 lg:col-span-1 min-w-0 w-full"
          />
          <select name="type" defaultValue={type || ''} className="rounded-xl border px-3 py-2.5 text-sm w-full min-w-0">
            <option value="">All types</option>
            <option value="CHANNEL">Channels</option>
            <option value="GROUP">Groups</option>
          </select>
          <select name="lang" defaultValue={lang || ''} className="rounded-xl border px-3 py-2.5 text-sm w-full min-w-0">
          ...
          <select
            ...
            className="rounded-xl border px-3 py-2.5 text-sm w-full min-w-0"
          >
          ...
          <select name="min" defaultValue={min ? String(min) : ''} className="rounded-xl border px-3 py-2.5 text-sm w-full min-w-0">
          ...
          <select name="sort" defaultValue={sort} className="rounded-xl border px-3 py-2.5 text-sm w-full min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {entities.map((e) => (
            <Link
              key={e.id}
              href={'/entity/' + e.id}
              className="bg-white rounded-2xl border border-slate-200 p-4 hover:border-[#c41e3a]/40 hover:shadow-sm transition min-w-0 overflow-hidden"
            >
        ...
        <div className="mt-8 flex flex-wrap gap-2 text-xs">
          <Link href={href({ type: 'CHANNEL' })} className="px-3 py-1 rounded-full bg-white border">
            Channels
          </Link>
          <Link href={href({ type: 'GROUP' })} className="px-3 py-1 rounded-full bg-white border">
            Groups
          </Link>
          <Link href={href({ min: '10000' })} className="px-3 py-1 rounded-full bg-white border">
            10K+
          </Link>
          <Link href="/explore" className="px-3 py-1 rounded-full bg-white border">
            Clear
          </Link>
        </div>
      </div>
